...
type BuildPasswordResetEmailArgs = {
  code: string;
  resetUrl: string;
  expiryMinutes: number;
};

/** Splits an 8-digit code into individual characters for display boxes. */
function codeDigits(code: string): string[] {
  return code.padStart(8, '0').slice(0, 8).split('');
}

export function buildPasswordResetEmail(
  args: BuildPasswordResetEmailArgs,
): PasswordResetEmailContent {
  const { code, resetUrl, expiryMinutes } = args;
  ...
  const subject = 'Your DocuShield password reset code';

  const text = [
    'DocuShield — Password reset',
    '',
    'We received a request to reset your password.',
    '',
    `Your reset code: ${code}`,
    '',
    `Open this link to enter your code: ${resetUrl}`,
    '',
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="max-width:520px;background-color:#ffffff;border-radius:12px;border:1px solid #e4ebf3;overflow:hidden;">
          ...
          <tr>
            <td style="padding:32px 32px 24px;">
              <h1 style="margin:0 0 12px;font-size:20px;font-weight:700;color:#0c2b4e;line-height:1.3;">Reset your password</h1>
              <p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:#5a6b7b;">
                We received a request to reset the password for your DocuShield account. Enter the code below on the reset page, or use the button to continue.
              </p>
              <!-- Code -->
              ...
              <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
                <tr>
                  <td align="center" style="padding:0 0 24px;">
                    <a href="${resetUrl}" style="display:inline-block;background-color:#0c2b4e;color:#f4f4f4;text-decoration:none;font-size:14px;font-weight:600;padding:14px 28px;border-radius:8px;">Reset password</a>
                  </td>
                </tr>
              </table>
              <!-- Expiry -->
